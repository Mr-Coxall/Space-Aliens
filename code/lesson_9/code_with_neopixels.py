...
def game_scene():
    ...
    # turn on Neopixels
    pixels = neopixel.NeoPixel(board.NEOPIXEL, constants.TOTAL_NUMBER_OF_LASERS, auto_write = False)
    for a_neopixel in range(0, constants.TOTAL_NUMBER_OF_LASERS):
        pixels[a_neopixel] = (0, 10, 0)
    pixels.show()
    ...
    game = stage.Stage(ugame.display, constants.FPS)
    # set the layers, items show up in order
    game.layers = lasers + sprites + [background]
    # render the background and initial location of sprite list
    # most likely you will only render background once per scene
    game.render_block()
    ...
    while True:
        ...
        # update Neopixels
        # turn all green to start with
        for a_neopixel in range(0, constants.TOTAL_NUMBER_OF_LASERS):
            pixels[a_neopixel] = (0, 10, 0)
        # now change the ones to yellow that are moving
        for a_neopixel in range(0, lasers_moving):
            pixels[a_neopixel] = (10, 10, 0)   
        # if 5 are moving, make all red
        if lasers_moving >= 5:
            for a_neopixel in range(0, constants.TOTAL_NUMBER_OF_LASERS):
                pixels[a_neopixel] = (10, 0, 0)  
        # show Neopixels
        pixels.show()
        
        
        # redraw sprite list
        game.render_sprites(lasers + sprites)
        game.tick() # wait until refresh rate finishes
